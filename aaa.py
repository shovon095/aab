...
# Define the image normalization and padding settings
img_norm_cfg = dict(
...
train_pipeline = [
    ...
    dict(type='Resize', img_scale=(1024, 1024), keep_ratio=True), # Match chip size
...
# Construct the final data dictionary, now including the pipeline
data = dict(
    samples_per_gpu=1, # Kept at 1 to prevent memory issues
    workers_per_gpu=2,
    train=dict(
        type=dataset_type,
        ann_file=data_root + 'Annotations/train.json',
        img_prefix=data_root + 'Images/train/',
        pipeline=train_pipeline), # <<< THE MISSING PIECE IS ADDED HERE
    val=dict(
        type=dataset_type,
        ann_file=data_root + 'Annotations/train.json',
        img_prefix=data_root + 'Images/train/',
        pipeline=test_pipeline), # <<< THE MISSING PIECE IS ADDED HERE
    test=dict(
        type=dataset_type,
        ann_file=data_root + 'Annotations/train.json',
        img_prefix=data_root + 'Images/train/',
